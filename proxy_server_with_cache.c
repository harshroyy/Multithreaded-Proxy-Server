...
// Note: caller (add_cache_element) must hold 'lock' before calling this function.
void remove_cache_element() {
    if (head == NULL) {
        return;
    }
...
int add_cache_element(char* data, int size, char* url) {
    ...
    if(element_size > MAX_ELEMENT_SIZE) {
        ...
        memcpy(element->data, data, size);      // BUG FIX #5 (partial): use memcpy not strcpy so binary data is safe
        element->data[size] = '\0';
...
int sendErrorMessage(int socket, int status_code) {
    ...
    switch(status_code) {
        ...
        case 405:
            ...
            sprintf(str, "HTTP/1.1 405 Method Not Allowed\r\nContent-Type: text/html\r\nConnection: close\r\nDate: %s\r\n\r\n"
                "<html><head><title>405 Method Not Allowed</title></head><body>"
                "<h1>405 Method Not Allowed</h1><p>Only GET method is supported.</p></body></html>\r\n",
                currentTime);
            break;
        case 500:
...
int connectRemoteServer(char* host_addr, int port_num)
{
	...
	{
        // perror prints our string, PLUS the exact system reason (e.g., "Connection refused")
...
int handle_request(int client_socketId, ParsedRequest *request, char* tempReq) {
    ...
    printf("handle_request: Connected to remote server. Sending reconstructed request...\n");

    printf("Before send\n");
    int bytes_send = send(remoteSocketId, buf, strlen(buf), 0);
    printf("After send\n");
    printf("handle_request: Sent %d bytes to remote server. Waiting for response...\n", bytes_send);
    bzero(buf, MAX_BYTES);
...
int main(int argc, char* argv[]) {
    ...
    while(1) {
        ...
        pthread_create(&tid[i % MAX_CLIENTS], NULL, thread_fn, (void *)&Connected_socketId[i % MAX_CLIENTS]);
        i++;
    }
